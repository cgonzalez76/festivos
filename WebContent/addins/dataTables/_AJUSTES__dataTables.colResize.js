        ...
        ColResize.prototype = {
            ...
            "_fnSetupMouseListeners": function () {
                ...
                $(that.s.dt.nTableWrapper).off("mouseenter.ColResize").on("mouseenter.ColResize", "th", function (e) {
					...
					$.each(filas, function(keytr, valuetr){
										   
						var tds=valuetr.cells;
						
						$.each(tds, function(keytd, valuetd){
												 
							if($(valuetd).index() == tmpth.index()){
							
								$(valuetd).css('white-space', 'normal');
							}
            ...
            "_fnMouseDown": function (e, nTh) {
                ...
                var thList = that.dom.resizeCol.closest('tr').find('th');
				var idTableParent = that.dom.resizeCol.closest('table').attr('id');
                ...
                $(document)
                    .off('mousemove.ColResize').on('mousemove.ColResize', function (e) {
                        that._fnMouseMove.call(that, e, thList, idTableParent);
                    })
                    .off('mouseup.ColResize').on('mouseup.ColResize', function (e) {
                        that._fnMouseUp.call(that, e, thList, idTableParent);
                    });
            },
             ...
             *  @private
             */
            "_fnMouseMove": function (e, thList, idTableParent) {
                var that = this;
                ...
                if ((dx > 0 && distFromRight <= 0) || (dx < 0 && distFromRight >= 0)) {
                    if (!that.s.init.tableWidthFixed) {
						...
						var domCols=[];
						if(typeof idTableParent !== 'undefined' && idTableParent!=null && idTableParent!=''){
									domCols = $(that.s.dt.nTableWrapper).find("table[id='" + idTableParent + "'] > thead > tr ").find("th[data-column-index='" + colResizeIdx + "']");
								
						}
						
						console.log(domCols);
						
						/*domCols.each(function(){
									//console.log($(this).closest('table')[0].id);
									
									if($(this).closest()==colResizeIdx){
										domCols.push(this);
									}
						});*/
						...
						//domCols.parents("table").css('background-color', '#F00');
						
                        domCols.parents("table").each(function () {
                            if (!$(this).parent().hasClass("DTFC_LeftBodyLiner")) {
								console.log(idTableParent + ' : ' + $(this).attr('id'));
