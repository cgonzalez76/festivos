...
uis.controller('uiSelectCtrl',
  ...
  ctrl.removeSelected = true; //If selected item(s) should be removed from dropdown list ESTABA A FALSE
  ctrl.closeOnSelect = uiSelectConfig.closeOnSelect;; //Initialized inside uiSelect directive link function
  ...
  ctrl.activate = function(initSearchValue, avoidReset) {
    if (!ctrl.disabled  && !ctrl.open) {
	  ...
	  //RRS PARCHE 20160222 porque lanzaba errores si se pulsaba borrar elementos seleccionados estando abierto el combo listado 
		  ...
		  if (ctrl.$animate && ctrl.$animate.enabled(container[0])) {
			ctrl.$animate.on('enter', container[0], function (elem, phase) {
			  if (phase === 'close') {
				...
				$timeout(function () {
				  //ctrl.focusSearchInput(initSearchValue);  //CAMBIO AJUSTE RRS
				});
			  }
			});
		  } else {
  ...
  ctrl.select = function(item, skipFocusser, $event) {
    if (item === undefined || !item._uiSelectChoiceDisabled) {
      ...
      if (!item || !item._uiSelectChoiceDisabled) {
        ...
        $scope.$broadcast('uis:select', {item:item, event:$event});
  ...
  function _ensureHighlightVisible() {
	...
	//RRS PARCHE 20160222 porque lanzaba errores si se pulsaba borrar elementos seleccionados estando abierto el combo listado
	if(typeof highlighted!=='undefined' && highlighted!=null){ //AJUSTE COMPROBACION RRS
  ...
  return {
    ...
    compile: function(tElement, tAttrs) {
      ...
      return function(scope, element, attrs, ctrls, transcludeFn) {
		...
		attrs.$observe('closeOnSelect', function() {
            var closeOnSelect = scope.$eval(attrs.closeOnSelect);
            $select.closeOnSelect = closeOnSelect !== undefined ? closeOnSelect : uiSelectConfig.closeOnSelect;
			//console.log('$select.closeOnSelect');
        });
  ...
  return {
    ...
    link: function(scope, element, attrs, ctrls) {
      ...
      scope.$on('uis:select', function (event, item) {
		...
		//RRS PARCHE 20160222 para selecciones multiples ya sea con filtro o sin el se necesitaba que si se pulsaba control-key no se cerrase el lista combo y poder seguir seleccionando
		if (item.event.ctrlKey){
			$select.closeOnSelect = false;
			$select.selected.push(item.item);
